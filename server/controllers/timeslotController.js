...
const generateDayTimeslots = (date) => {
  const slots = [];
  const startHours = [8, 10, 12, 14, 16, 18]; // 8am to 6pm, 2-hour slots
...
exports.assignTimeslot = async (req, res) => {
  try {
    ...
    timeslot.assignedUser = userId || null;
    timeslot.notes = notes || "";
    await timeslot.save();

    await timeslot.populate("assignedUser", "name email");

    res.json({
      success: true,
      msg: userId ? "Timeslot assigned successfully" : "Timeslot unassigned successfully",
      timeslot,
    });
  } catch (error) {
